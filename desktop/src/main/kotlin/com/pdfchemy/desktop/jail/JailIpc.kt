    ...
    fun sendChunked(
        ...
        if (sourceFile != null && sourceFile.length() > JailIpc.MAX_IPC_FRAME_SIZE) {
            ...
            val startSigned = startReq.copy(
                signature = JailCrypto.computeSignature(secret, sessionId, jobId, "worker", "res", "START", 0, totalSize, 0L, "", startNonce)
            )
            ...
            sourceFile.inputStream().use { fileIn ->
                ...
                while (true) {
                    ...
                    val chunkSigned = chunkReq.copy(
                        signature = JailCrypto.computeSignature(secret, sessionId, jobId, "worker", "res", "CHUNK", seq, totalSize, chunkBytes.size.toLong(), payloadHash, chunkNonce)
                    )
            ...
            val endSigned = endReq.copy(
                signature = JailCrypto.computeSignature(secret, sessionId, jobId, "worker", "res", "END", seq, totalSize, 0L, "", endNonce)
            )
            ...
            val singleSigned = singleReq.copy(
                signature = JailCrypto.computeSignature(secret, sessionId, jobId, "worker", "res", "SINGLE", 0, payloadLen, payloadLen, payloadHash, nonce)
            )
