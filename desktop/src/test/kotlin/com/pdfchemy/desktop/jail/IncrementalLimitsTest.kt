...
fun main(args: Array<String>) {
    if (args.contains("HOSTILE_STDERR")) {
        ...
        val startSigned = startReq.copy(
            signature = JailCrypto.computeSignature(secret, sessionId, jobId, "worker", "res", "START", 0, 5L * 1024 * 1024, 0L, "", startNonce)
        )
        ...
        while (true) {
            ...
            val chunkSigned = chunkReq.copy(
                signature = JailCrypto.computeSignature(secret, sessionId, jobId, "worker", "res", "CHUNK", seq, 5L * 1024 * 1024, chunkBytes.size.toLong(), payloadHash, nonce)
            )
