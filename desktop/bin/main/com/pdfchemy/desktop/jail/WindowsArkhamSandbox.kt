...
class WindowsArkhamSandbox(private val launcherExePath: Path) : ArkhamSandbox {
    ...
    private var cleanupState: CleanupState? = null

    private data class CleanupState(
        val worker: VerifiedWorker,
        val sidGrant: String,
        val jreDir: java.io.File
    )
    ...
    override fun launch(worker: VerifiedWorker): SandboxedProcess {
        ...
        fun runIcacls(vararg args: String) {
            val pb = ProcessBuilder(*args)
                ...
                .start()
            if (pb.waitFor() != 0) {
                System.err.println("Warning: Failed to grant filesystem ACLs via icacls: ${args.joinToString(" ")}")
            }
        }
        
        val appContainerName = worker.environment["ARKHAM_SESSION"]?.let { "ArkhamWorker_$it" } ?: "ArkhamAsylumWorker"

        val sidPb = ProcessBuilder(launcherExePath.toAbsolutePath().toString(), "--get-sid", appContainerName).start()
        val sidStr = sidPb.inputStream.bufferedReader().use { it.readText().trim() }
        val sidGrant = if (sidPb.waitFor() == 0 && sidStr.isNotEmpty()) {
            "*$sidStr"
        } else {
            "*S-1-15-2-1" // Fallback to ALL APPLICATION PACKAGES
        }
        ...
        cleanupState = CleanupState(worker, sidGrant, jreDir)
        ...
        pb.environment()["ARKHAM_APP_CONTAINER_NAME"] = appContainerName
    ...
    override fun terminate() {
        ...
        val state = cleanupState ?: return
        val systemRoot = System.getenv("SystemRoot") ?: "C:\\Windows"
        val icaclsPath = "$systemRoot\\System32\\icacls.exe"
        try {
            runIcacls(icaclsPath, state.jreDir.absolutePath, "/remove", state.sidGrant)
            runIcacls(icaclsPath, state.worker.classPath, "/remove", state.sidGrant)
            runIcacls(icaclsPath, state.worker.workingDirectory.absolutePath, "/remove", state.sidGrant)
        } catch (e: Exception) {
            System.err.println("Failed to revoke ACLs: ${e.message}")
        }
    }
        ...
        while (System.currentTimeMillis() - startTime < 60000) {
            if (errorStream.available() > 0) {
                ...
                if (!p.isAlive) {
                    ...
                    while (true) {
                        val b = try { errorStream.read() } catch (e: Exception) { -1 }
                        if (b == -1) break
                        val c = b.toChar()
                        if (c == '\n') {
                            val line = lineBuffer.toString().trim()
